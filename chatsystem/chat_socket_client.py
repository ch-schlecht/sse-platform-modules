...
the_chat_websocket_client = None
async def get_socket_instance():
    global the_chat_websocket_client
    if the_chat_websocket_client is None:
        the_chat_websocket_client = Client(tornado.httpclient.HTTPRequest("wss://localhost:" + str(CHAT_CONSTANTS.PLATFORM_PORT) + "/websocket", validate_cert=False))
        await the_chat_websocket_client._await_init()
    return the_chat_websocket_client
